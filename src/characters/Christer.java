...
public class Christer extends Person implements Throwing {
    ...
    public void bring(Character chr) throws OwnerException {
        if (chr instanceof Pet) {
            Pet pet = (Pet) chr;
            if (pet.belongTo(this))
                System.out.println(getName() + " привёл своего щенка по имени " + pet.getName() + '.');
            else throw new OwnerException();
        } else System.out.println(getName() + " и " + chr.getName() + " пришли вдвоём.");
    }

    public void standBy(Character... chr) {
        if (chr.length == 1) {
            System.out.println(getName() + " стоял рядом с " + chr[0].getName());
    ...
    public void knowWhoOwner(Pet pet) {
        if (pet.belongTo(this))
            System.out.println(getName() + " знал, что " + pet.getName() + " -- это его собака, а не чья-нибуь ещё.");
        else
            System.out.println(getName() + " знал, что владелец собаки по имени " + pet.getName() + " -- это " + pet.getOwnerName() + ".");
    }
